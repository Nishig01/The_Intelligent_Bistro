...
import { X, CreditCard, Apple, ArrowRight, Loader2, CheckCircle2, ShieldCheck, ChevronLeft } from 'lucide-react';
    ...
    try {
      ...
      await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          items: items,
          total: total,
          customerName: user?.name || name || 'Valued Guest',
          email: user?.email
        })
      });
    } catch (e) {
    ...
    >
      ...
      <div className="flex justify-between items-center px-6 pb-4 pt-2 border-b border-gray-100 bg-white shadow-sm shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h2 className="font-serif text-2xl text-bistro-charcoal">Checkout</h2>
        </div>
        <button onClick={onClose} disabled={paymentState !== 'idle'} className="p-2 -mr-2 rounded-full hover:bg-gray-100 transition-colors disabled:opacity-50">
        ...
        <div>
          ...
          <div className="flex gap-3">
            <button 
              ...
              className="flex-1 bg-black text-white rounded-xl py-3.5 flex items-center justify-center shadow-sm hover:shadow-md transition-all active:scale-95 disabled:opacity-50"
            >
            ...
            <button 
              ...
              disabled={paymentState !== 'idle'}
              className="flex-1 bg-white border border-gray-200 text-black rounded-xl py-3.5 flex items-center justify-center shadow-sm hover:shadow-md transition-all active:scale-95 disabled:opacity-50"
            >
